  ...
  if (!isLoggedIn) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <Link href="/" className="inline-flex items-center gap-2 bg-[#1e3a5f] text-white px-6 py-3 rounded-xl font-semibold hover:bg-[#162d4a] transition-colors">
          Go to calculators
        </Link>
      </div>
    );
            ...
            return (
              <div key={type}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  {meta && (
                    <Link href={meta.href} className="flex items-center gap-1 text-sm text-blue-600 hover:underline">
                  ...
                  <ul className="divide-y divide-gray-50">
                    {items.map((calc) => {
                      const inputPreview = Object.entries(calc.input_data as Record<string, unknown>)
                        ...
                        .join(" · ");
                      return (
                        <li key={calc.id} className="flex items-start justify-between gap-4 px-5 py-4">
